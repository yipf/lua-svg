...
local draw_stack={}
local EDGE_KEY,PATH_KEY="EDGE","PATH"
...
path=function(p)
	if type(p)~="table" or #p<2 then return end
	p.TYPE=PATH_KEY
	push(draw_stack,p)
	return p,#draw_stack
end
...
--~ 	if close then table.insert(points,points[1]) end
...
prepare_map=function()
	...
	return function(f)
		...
		if s<e and type(f)=="function" then 
			for i=s+1,e do
				obj=all[i]
				if templates[obj.TYPE] then -- if a valid node 
					f(obj) 
				end
			end
		end
...
export=function(filepath,png)
	...
	for i=1,#draw_stack do
		...
		if d.TYPE==EDGE_KEY then 	-- if an edge object
			points,d.cx,d.cy=shape2points(d.SHAPE,d.FROM,d.TO,d.OFFSET)
			d.PATH=points2str(points,d.SMOOTH,d.CLOSE)
			str=obj2str(d,"path")
		elseif d.TYPE==PATH_KEY then 	-- if an path object
			for i,v in ipairs(d) do
				if v.TYPE then v[1],v[2]=v.cx,v.cy end
			end
			d.PATH=points2str(d,d.SMOOTH,d.CLOSE)
			str=obj2str(d,"path")
